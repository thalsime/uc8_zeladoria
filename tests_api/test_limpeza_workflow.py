...
def test_iniciar_limpeza_zelador_sucesso(
    api_base_url: str,
    ...
    assert response_data["funcionario_responsavel"] == os.getenv(
        "TEST_USER_ZELADOR_USERNAME"
    )
...
def test_iniciar_limpeza_outros_usuarios_falha(
    ...
    response = requests.post(
        f"{api_base_url}/salas/{sala_uuid}/iniciar_limpeza/", headers=header
    )
    assert response.status_code == 403


def test_iniciar_limpeza_ja_em_andamento_falha(
    api_base_url: str,
    auth_header_zelador: Dict[str, str],
...
def test_iniciar_limpeza_sala_inativa_falha(
    api_base_url: str,
    ...
    assert (
        "Salas inativas não podem ter a limpeza iniciada"
        in response_iniciar.json().get("detail", "")
    )
...
def test_adicionar_foto_zelador_sucesso(
    api_base_url: str,
    auth_header_zelador: Dict[str, str],
    iniciar_limpeza_para_teste: Dict[str, Any],
...
def test_adicionar_mais_de_tres_fotos_falha(
    api_base_url: str,
    auth_header_zelador: Dict[str, str],
    iniciar_limpeza_para_teste: Dict[str, Any],
    test_image_path: Path,
    ...
    for i in range(3):
        with open(test_image_path, "rb") as image_file:
            ...
            assert (
                response_add.status_code == 201
            ), f"Falha ao adicionar foto {i+1}: {response_add.text}"
...
def test_adicionar_foto_outros_usuarios_falha(
    api_base_url: str,
    request: Any,
    auth_fixture: str,
    iniciar_limpeza_para_teste: Dict[str, Any],
...
def test_concluir_limpeza_zelador_com_foto_sucesso(
    api_base_url: str,
    auth_header_zelador: Dict[str, str],
    iniciar_limpeza_para_teste: Dict[str, Any],
    test_image_path: Path,
    ...
    with open(test_image_path, "rb") as image_file:
        files = {"imagem": (test_image_path.name, image_file, "image/png")}
        response_add_foto = requests.post(
            f"{api_base_url}/fotos_limpeza/",
            headers=auth_header_zelador,
            data=payload_foto,
            files=files,
    ...
    assert (
        response_concluir.status_code == 200
    ), f"Falha ao concluir limpeza: {response_concluir.text}"
    ...
    assert len(response_data["fotos"]) == 1


def test_concluir_limpeza_sem_foto_falha(
    api_base_url: str,
    auth_header_zelador: Dict[str, str],
    iniciar_limpeza_para_teste: Dict[str, Any],
):
...
def test_concluir_limpeza_sem_iniciar_falha(
    api_base_url: str,
    auth_header_zelador: Dict[str, str],
    sala_de_teste: Dict[str, Any],
):
...
def test_concluir_limpeza_outros_usuarios_falha(
    api_base_url: str, request: Any, auth_fixture: str, sala_de_teste: Dict[str, Any]
):
    ...
    header = request.getfixturevalue(auth_fixture)
    response = requests.post(
        f"{api_base_url}/salas/{sala_uuid}/concluir_limpeza/", headers=header, json={}
    )
    assert response.status_code == 403
