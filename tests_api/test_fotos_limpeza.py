...
def setup_fotos_para_listagem(
    ...
    auth_header_assistente: Dict[str, str], # Usaremos o 'assistente' como segundo zelador
    ...
    zelador2_username = os.getenv("TEST_USER_ASSISTENTE_USERNAME") # Usando assistente

    salas_criadas_uuids = []
    registros_limpeza_ids = {} # {username: registro_id}
    fotos_criadas = {} # {username: foto_id}

    # 1. Criar duas salas únicas usando o admin
    for i in range(2):
    ...
    # 2. Função auxiliar para simular limpeza com foto
    def _simular_limpeza_com_foto(sala_uuid: str, auth_header: Dict[str, str], zelador_username: str):
        # Iniciar Limpeza
        resp_iniciar = requests.post(f"{api_base_url}/salas/{sala_uuid}/iniciar_limpeza/", headers=auth_header)
        assert resp_iniciar.status_code == 201, f"Fixture: Falha ao iniciar limpeza por {zelador_username}"
        ...
        with open(test_image_path, "rb") as img:
            ...
            resp_foto = requests.post(f"{api_base_url}/fotos_limpeza/", headers=auth_header, data=data, files=files)
            assert resp_foto.status_code == 201, f"Fixture: Falha ao adicionar foto por {zelador_username}"
            fotos_criadas[zelador_username] = resp_foto.json()["id"]

        # Concluir Limpeza
        resp_concluir = requests.post(f"{api_base_url}/salas/{sala_uuid}/concluir_limpeza/", headers=auth_header)
        assert resp_concluir.status_code == 200, f"Fixture: Falha ao concluir limpeza por {zelador_username}"

    # 3. Executar o fluxo para cada zelador em salas diferentes
    _simular_limpeza_com_foto(salas_criadas_uuids[0], auth_header_zelador, zelador1_username)
    _simular_limpeza_com_foto(salas_criadas_uuids[1], auth_header_assistente, zelador2_username)
    ...
    # 4. Limpeza: deletar as salas criadas (fotos e registros serão deletados em cascata)
    for sala_uuid in salas_criadas_uuids:
        # Reativar a sala antes de deletar, caso algum teste a tenha desativado
...
# Testes de Listagem (GET /api/fotos_limpeza/)
...
def test_listar_fotos_como_solicitante_lista_vazia(api_base_url: str, auth_header_solicitante: Dict[str, str], setup_fotos_para_listagem: Dict[str, Any]):
    """Verifica se Solicitante recebe 200 OK com uma lista vazia."""
    response = requests.get(f"{api_base_url}/fotos_limpeza/", headers=auth_header_solicitante)
...
def test_listar_fotos_como_zelador_proprio(
    ...
    response = requests.get(f"{api_base_url}/fotos_limpeza/", headers=auth_header_zelador)
    assert response.status_code == 200
    fotos_listadas = response.json()

    # IDs esperados e não esperados
    foto_id_propria = setup_fotos_para_listagem["foto_id_zelador1"]
    foto_id_outro = setup_fotos_para_listagem["foto_id_zelador2"]

    assert len(fotos_listadas) > 0 # Garante que a fixture criou a foto esperada

    ids_listados = {foto['id'] for foto in fotos_listadas}

    assert foto_id_propria in ids_listados
    assert foto_id_outro not in ids_listados # Crucial: não deve ver a foto do outro zelador
...
def test_listar_fotos_como_admin(
    ...
    assert len(fotos_listadas) >= 2 # Deve conter pelo menos as duas da fixture

    ids_listados = {foto['id'] for foto in fotos_listadas}

    # Verifica se ambas as fotos criadas pela fixture estão na lista
...
def test_recuperar_foto_nao_autenticado_falha(api_base_url: str, setup_fotos_para_listagem: Dict[str, Any]):
    """Verifica se acesso não autenticado é negado (401)."""
...
def test_recuperar_foto_como_solicitante_falha(
    ...
    foto_id = setup_fotos_para_listagem["foto_id_zelador1"]
    response = requests.get(f"{api_base_url}/fotos_limpeza/{foto_id}/", headers=auth_header_solicitante)
    # Espera 404 porque o get_queryset filtra antes
...
def test_recuperar_foto_propria_como_zelador(
    ...
    foto_id_propria = setup_fotos_para_listagem["foto_id_zelador1"]
    response = requests.get(f"{api_base_url}/fotos_limpeza/{foto_id_propria}/", headers=auth_header_zelador)
    assert response.status_code == 200
    foto_data = response.json()
    assert foto_data["id"] == foto_id_propria
    assert "imagem" in foto_data # Verifica se a URL da imagem está presente
...
def test_recuperar_foto_outro_zelador_falha(
    ...
    foto_id_outro = setup_fotos_para_listagem["foto_id_zelador2"]
    response = requests.get(f"{api_base_url}/fotos_limpeza/{foto_id_outro}/", headers=auth_header_zelador)
    assert response.status_code == 404
...
def test_recuperar_foto_como_admin(
    ...
    # Testa com a foto do zelador 1
    response1 = requests.get(f"{api_base_url}/fotos_limpeza/{foto_id_zelador1}/", headers=auth_header_admin)
    assert response1.status_code == 200
    foto_data1 = response1.json()
    assert foto_data1["id"] == foto_id_zelador1

    # Testa com a foto do zelador 2
    response2 = requests.get(f"{api_base_url}/fotos_limpeza/{foto_id_zelador2}/", headers=auth_header_admin)
...
def test_excluir_foto_nao_autenticado_falha(api_base_url: str, setup_fotos_para_listagem: Dict[str, Any]):
    """Verifica se acesso não autenticado é negado (401)."""
    foto_id = setup_fotos_para_listagem["foto_id_zelador1"]
...
def test_excluir_foto_como_solicitante_falha(
    ...
    foto_id = setup_fotos_para_listagem["foto_id_zelador1"]
    response = requests.delete(f"{api_base_url}/fotos_limpeza/{foto_id}/", headers=auth_header_solicitante)
    # Espera 404 porque o get_queryset filtra antes
    assert response.status_code == 404
...
def test_excluir_foto_outro_zelador_falha(
    ...
    foto_id_outro = setup_fotos_para_listagem["foto_id_zelador2"]
    response = requests.delete(f"{api_base_url}/fotos_limpeza/{foto_id_outro}/", headers=auth_header_zelador)
    assert response.status_code == 404
...
def test_excluir_foto_propria_como_zelador(
    ...
    auth_header_zelador: Dict[str, str], # Logado como zelador1
    setup_fotos_para_listagem: Dict[str, Any],
    request # Necessário para adicionar finalizador
):
    """Verifica se Zelador pode excluir sua própria foto."""
    foto_id_propria = setup_fotos_para_listagem["foto_id_zelador1"]

    # Ação
    response_delete = requests.delete(f"{api_base_url}/fotos_limpeza/{foto_id_propria}/", headers=auth_header_zelador)

    # Assertiva
    assert response_delete.status_code == 204

    # Verificação
    # Tenta buscar a foto excluída, espera 404
    response_get = requests.get(f"{api_base_url}/fotos_limpeza/{foto_id_propria}/", headers=auth_header_zelador)
    assert response_get.status_code == 404

    # Limpeza Adicional (Recriar a foto para não afetar outros testes que usam a fixture)
    # A fixture `setup_fotos_para_listagem` tem escopo de função,
    # então a foto será recriada na próxima vez que for usada.
    # Não precisamos de limpeza manual aqui.
...
def test_excluir_foto_como_admin(
    ...
    # Ação
    response_delete = requests.delete(f"{api_base_url}/fotos_limpeza/{foto_id_zelador2}/", headers=auth_header_admin)

    # Assertiva
    assert response_delete.status_code == 204

    # Verificação
    # Tenta buscar a foto excluída como Admin, espera 404
    response_get = requests.get(f"{api_base_url}/fotos_limpeza/{foto_id_zelador2}/", headers=auth_header_admin)
    assert response_get.status_code == 404

    # Limpeza Adicional
    # A fixture cuidará da limpeza ao final da sua execução.


# Testes de Permissão de Criação (POST /api/fotos_limpeza/)

def test_criar_foto_nao_autenticado_falha(
    api_base_url: str,
    test_image_path: Path
):
    """Verifica se não autenticado recebe 401 ao tentar criar foto."""
    payload = {"registro_limpeza": "1"} # ID do registro não importa aqui
...
def test_criar_foto_permissoes_invalidas_falha(
    ...
    auth_fixture: str,
    test_image_path: Path,
    iniciar_limpeza_para_teste: Dict[str, Any] # Garante um registro válido
):
    """Verifica se Admin e Solicitante recebem 403 ao tentar criar foto."""
    header = request.getfixturevalue(auth_fixture)
    registro_id = iniciar_limpeza_para_teste["id"] # Usa um ID de registro real
